# Phase 2B-R5 Preregistered Laboratory Contract — Revision E.2
...
**Freeze timestamp:** `2026-09-05T05:31:46.290Z`  
**Supersedes Revision E.1:** SHA-256 `F39371BF9A9314E03B63F3DC8F203FCF65AC2DE56E2E511658FDC343C11966E6`, 17,966 bytes  
**Supersession reason:** RECEIPT-SCHEMA-001 remained open — §10.0 field-authority classification still stated "fields 1–14" instead of "fields 1–14 and 22". Additionally, Revision E.1 contained mojibake encoding corruption (double-encoded UTF-8 em-dashes and section signs) introduced during the E→E.1 PowerShell copy/modify cycle.  
...
- **Root defect:** RECEIPT-AUTHORITY-001 — gate schema required 22 fields; contract §10 specified 11; producer emitted incompatible schema; orchestrator constructed replacement receipts with invented `attempt_id`; no preregistered authority for that repair
...
**E.1. Receipt Schema Bound Into Contract (§10 — REWRITTEN)**  
...
**E.2. Producer Is Authoritative Receipt Creator (§10.1 — NEW)**  
...
**E.3. Pre-Dispatch Receipt Identity Assignment (§10.2 — NEW)**  
`attempt_id`, `branch`, `producing_agent`, `baseline_ref`, `context_manifest_hash`, `authority_envelope`, and expected artifact paths are assigned BEFORE objective dispatch and included in the producer's work packet. The producer uses these exact values — it does not generate them. Defect addressed: RECEIPT-AUTHORITY-001.

**E.4. One Receipt Per Terminal Artifact (§10.3 — NEW)**  
...
**E.5. Producer Receipt Dry-Run Fixture (§10.4 — NEW)**  
...
**E.6. Receipt Schema Consistency Check Added to Baseline Audit (§18 — EXTENDED)**  
The final baseline audit now verifies receipt schema binding consistency. Defect addressed: RECEIPT-AUTHORITY-001.

**E.7. Eight Controlled Receipt Fixtures (§10.5 — NEW)**  
Eight pre-run fixtures prove the gate correctly accepts/rejects receipt variants. Defect addressed: RECEIPT-AUTHORITY-001.

**E.8. Experiment-Identity Consistency Check Retained (§16.1)**  
Carried forward unchanged from Revision D.

**E.9. Field-Authority Arithmetic Correction (§10.0 — Revision E.2)**  
§10.0 field-authority classification corrected: field 22 (`context_manifest_hash`) is pre-dispatch assigned and now explicitly included in the pre-dispatch range. Field 7 (`ledger_event`) source explicitly states pre-dispatch origin. Defect addressed: RECEIPT-SCHEMA-001.

All other rules (§§1–9, 11–15, 16, 17) are carried forward unchanged from Revision D.
...
Identical to Revision B §5 with §5.1 from Revision D.

### 5.1. Control-Validity Gate

Identical to Revision D §5.1. Unchanged.

## 6. Parallel definition and topology

Identical to Revision B §6. Unchanged.

## 7. Authority envelopes

Identical to Revision B §7. Kant and Kant sub-agents remain OUT OF SCOPE / NON-PARTICIPATING.

**R5 authority map:** `AUTHORITY-MAP-R5.json` — all agent path prefixes bound to `PHASE-2B-R5/...`. Verified by experiment-identity consistency check (§16.1).

## 8. Semantic acceptance contract

Identical to Revision D §8 including §8.1. Unchanged.

## 9. Signal Fire firewall

Identical to Revision B §9. Unchanged.

## 10. Receipt finalization contract (REWRITTEN — fixes RECEIPT-AUTHORITY-001 and RECEIPT-SCHEMA-001)

### 10.0. Canonical Gate Receipt Schema

Every receipt file validated by the integration gate MUST contain exactly these 22 fields. This is the canonical schema — it supersedes any prior "Receipt fields required" list.
...
| 3 | `generation` | integer ≥ 1 | Pre-dispatch assignment | Generation number |
...
- **Pre-dispatch assigned (fields 1–14 and 22):** These values are determined by the Global Orchestrator BEFORE the producer receives its objective. They are included verbatim in the producer's work packet. The producer copies them into the receipt without modification. Total: 15 fields.
- **Producer-determined (fields 15–21):** These values are generated by the producer from actual artifact bytes, file stats, and the producer's own verification process. The orchestrator may NOT invent, author, or substitute these fields. Total: 7 fields.
- **Deterministic machine-derived:** `provenance` and `bytes` MUST be computed from the final closed artifact file — not from memory, not from model-generated text, not from orchestrator readback.

**Arithmetic check:** 15 pre-dispatch + 7 producer-determined = 22 total fields. ✓
...
- **MAY** provide pre-dispatch field values for fields 1–14 and 22 as part of the work packet
...
- **MAY NOT** invent values for producer-determined fields (15–21)
...
10. If mismatch: set `verification_status` to `"HASH_MISMATCH"` — do NOT silently continue.
...
5. If a receipt is missing or malformed, the gate FAILS — this is a valid experimental result

## 11. Judge identity and evidence transport

Identical to Revision D §11 including §11.1 and §11.2. Unchanged.

## 12. Timing contract

Identical to Revision D §12. Unchanged.

## 13. Context-lifecycle precondition

Identical to Revision B §13. Unchanged.

## 14. Contamination rules

Identical to Revision D §14.

**Additional:** No R4 produced control outputs, receipts (original or corrected), integration reports, metrics, gate results, or semantic content may enter R5 producer context. R4 outputs are historical failure evidence only. This prohibition does **not** bar the explicitly frozen `P2BR2-BASELINE-20260902-A` source corpus in §2.

## 15. Stop rules

Identical to Revision D §15. Unchanged.

## 16. R5 pass bar

Phase 2B-R5 PASS requires all 18 (identical to Revision D §16):
...
Identical to Revision D §16.1. Unchanged except stale token list updated:
...
Identical to Revision B §17. Unchanged.

## 18. Baseline audit two-layer verification

Identical to Revision D §18.

**Additional (E.6):** The final baseline audit must also verify that the frozen receipt schema definition in §10.0 is consistent with the integration gate's actual validation logic. Specifically: every field the gate checks for existence must appear in §10.0's canonical schema table. If the gate requires a field not in §10.0, the audit FAILS.
