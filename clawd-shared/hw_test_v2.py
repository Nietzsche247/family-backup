...
FONT = "Fancy Jenna"
DIMENSION_ID = 1  # accepted by create-card on the first run
...
def find_key():
    for d in KEY_DIRS:
        for name in ("hw_key.txt", "hw_key.txt.txt", "hw_key", "keyring.md"):
            ...
            if os.path.exists(p):
                try:
                    ...
                    continue
                # keyring.md may hold more than the key; grab a 32-char hex token if present
                import re
                m = re.search(r"[A-Fa-f0-9]{32}", txt)
                if m:
                    return m.group(0)
                k = txt.strip()
                if k:
...
def main():
    key = find_key()
    if not key:
        print("Could not find the key. Put it in hw_key.txt in")
        print(r"  C:\Users\aaron\clawd-shared")
        return
    H = {"Authorization": key}
    ...
    print(txt[:2000], "\n")
    print("=== DONE ===")
    print("Send me the HTTP code and the response for step 4. Screenshot is fine.")
