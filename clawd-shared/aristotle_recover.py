...
def teardown() -> bool:
    # L30/L31 (2026-05-11): Disable the scheduled task FIRST so the 5-min
    # periodic trigger does not respawn the broken supervisor while we kill
    # processes. Re-enabled in bring_up_gateway(). Without this, every PID
    # we kill comes back within seconds and recovery never converges.
    ...
    try:
        ...
        warn(f"Disable-ScheduledTask failed (continuing): {e}")
...
def bring_up_gateway() -> bool:
    # L30/L31 (2026-05-11): Re-enable the scheduled task that teardown()
    # disabled. Without this, the supervisor + 5-min periodic-trigger
    # auto-recovery is lost after this script exits.
    ...
    for i in range(GATEWAY_BIND_TIMEOUT_S):
        ...
        if pid:
            ...
            break
    if not pid:
        # L31 FALLBACK (2026-05-11): task path did not bind -- the wrapper
        # (aristotle-gateway-task.cmd) can hang in its own kill loop when
        # zombie supervisors exist. Bypass by launching gateway.cmd directly.
        # The scheduled task is a convenience layer; gateway.cmd is the
        ...
        try:
            ...
            return False
        info(f"direct gateway.cmd launched, waiting up to {GATEWAY_BIND_TIMEOUT_S}s for port")
        for i in range(GATEWAY_BIND_TIMEOUT_S):
            time.sleep(1)
            pid = get_port_owner(GATEWAY_PORT)
            if pid:
                ok(f"port bound via direct launch on PID {pid} after {i + 1}s")
                break
        if not pid:
            err(f"gateway did not bind via direct launch either")
            return False
