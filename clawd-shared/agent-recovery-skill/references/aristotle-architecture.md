...
### Recovery
- Script: `C:\Users\aaron\clawd-shared\aristotle_recover.py` (v2, stdlib Python)
...
4. **Hung gateway** (port bound, HTTP dead) -> Kill node PID, supervisor restarts
5. **Config corruption** -> Restore from daily backup
6. **All providers rate-limited** -> Wait; check billing/quotas
7. **Machine failure** -> SSH remote recovery or physical access
...
# Full diagnostic
python C:\Users\aaron\clawd-shared\aristotle_recover.py --check -v --json
```
